...
export default function NovoPcmsoPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      medicoResponsavel:  fd.get('medicoResponsavel'),
      crm:                fd.get('crm') || null,
      clinica:            fd.get('clinica') || null,
      vigenciaInicial:    fd.get('vigenciaInicial'),
      vigenciaFinal:      fd.get('vigenciaFinal') || null,
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/sst/pcmso" className="flex items-center justify-center rounded-lg" style={{ width: 32, height: 32, background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <ArrowLeft size={14} style={{ color: 'var(--text-secondary)' }} />
        </Link>
        <div>
          <h1 className="text-xl font-extrabold" style={{ color: 'var(--text-primary)', letterSpacing: '-.3px' }}>Novo PCMSO</h1>
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Programa de Controle Médico de Saúde Ocupacional</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="rounded-xl p-6 space-y-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Médico Responsável *">
              <input name="medicoResponsavel" required placeholder="Nome completo do médico" className="form-input" />
            </FormField>
            <FormField label="CRM">
              <input name="crm" placeholder="Ex: CRM/SP 123456" className="form-input" />
            </FormField>
          </div>

          <FormField label="Clínica / Empresa Parceira">
            <input name="clinica" placeholder="Nome da clínica ou empresa parceira" className="form-input" />
          </FormField>

          <div className="grid grid-cols-3 gap-4">
            <FormField label="Vigência Inicial *">
              <input name="vigenciaInicial" type="date" required className="form-input" />
            </FormField>
            <FormField label="Vigência Final">
              <input name="vigenciaFinal" type="date" className="form-input" />
            </FormField>
        ...
        <div className="flex justify-end gap-3 mt-4">
          <Link href="/sst/pcmso" className="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-secondary)' }}>
            Cancelar
          </Link>
