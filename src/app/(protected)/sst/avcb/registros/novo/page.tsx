...
export default function NovoAvcbPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      areaProtegida:      fd.get('areaProtegida') || null,
    ...
    try {
      const res = await fetch('/api/sst/avcb', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      ...
      if (docsRef.current?.hasPending()) {
        ...
        if (!ok) setError('Registro salvo, mas houve falha ao enviar um ou mais documentos. Você pode tentar novamente na tela de detalhe.')
      }

      router.push(`/sst/avcb/registros/${data.id}`)
      router.refresh()
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/sst/avcb/registros" className="flex items-center justify-center rounded-lg" style={{ width: 32, height: 32, background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <ArrowLeft size={14} style={{ color: 'var(--text-secondary)' }} />
        </Link>
        <div>
          <h1 className="text-xl font-extrabold" style={{ color: 'var(--text-primary)', letterSpacing: '-.3px' }}>Novo AVCB / CLCB</h1>
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Auto de Vistoria ou Certificado de Licenciamento do Corpo de Bombeiros</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="rounded-xl p-6 space-y-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div className="grid grid-cols-3 gap-4">
            <FormField label="Número do AVCB *">
              <input name="numero" required placeholder="Ex: 123456" className="form-input" />
            </FormField>
            <FormField label="Tipo *">
              <select name="tipo" className="form-select" defaultValue="AVCB">
                <option value="AVCB">AVCB</option>
                <option value="CLCB">CLCB</option>
              </select>
            </FormField>
            <FormField label="Órgão Emissor *">
              <input name="orgaoEmissor" required placeholder="Ex: Corpo de Bombeiros SP" className="form-input" />
            </FormField>
          </div>

          <div className="grid grid-cols-3 gap-4">
            <FormField label="Data de Emissão">
              <input name="dataEmissao" type="date" className="form-input" />
            </FormField>
            <FormField label="Data de Validade *">
              <input name="dataValidade" type="date" required className="form-input" />
            </FormField>
            <FormField label="Área Protegida (m²)">
              <input name="areaProtegida" type="number" step="0.01" placeholder="Ex: 1200" className="form-input" />
            </FormField>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <FormField label="Responsável Técnico">
              <input name="responsavelTecnico" placeholder="Nome do engenheiro / técnico" className="form-input" />
            </FormField>
        ...
        <div className="flex justify-end gap-3 mt-4">
          <Link href="/sst/avcb/registros" className="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-secondary)' }}>
            Cancelar
          </Link>
          <button type="submit" disabled={saving} className="flex items-center gap-2 rounded-lg px-5 py-2 text-sm font-bold text-white" style={{ background: 'var(--brand-gradient)', opacity: saving ? .7 : 1 }}>
            <Save size={14} />
            {saving ? 'Salvando...' : 'Salvar AVCB/CLCB'}
          </button>
        </div>
      </form>

      <div className="mt-4">
        <DocumentManager ref={docsRef} entidade="AVCB" tipos={TIPOS_DOCUMENTO_AVCB} titulo="Documentos" />
      </div>
    </div>
  )
}
