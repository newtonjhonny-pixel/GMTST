...
export default function NovaBrigadaPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      qtdMembros:      fd.get('qtdMembros') || null,
      dataTreinamento: fd.get('dataTreinamento') || null,
      dataValidade:    fd.get('dataValidade'),
      status:          fd.get('status'),
      observacao:      fd.get('observacao') || null,
    }
    try {
      const res = await fetch('/api/sst/brigada', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      const data = await res.json()
      if (!res.ok) { setError(data.error ?? 'Erro ao salvar'); setSaving(false); return }
      router.push('/sst/avcb/brigada')
      router.refresh()
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/sst/avcb/brigada" className="flex items-center justify-center rounded-lg" style={{ width: 32, height: 32, background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <ArrowLeft size={14} style={{ color: 'var(--text-secondary)' }} />
        </Link>
        <div>
          <h1 className="text-xl font-extrabold" style={{ color: 'var(--text-primary)', letterSpacing: '-.3px' }}>Nova Brigada de Incêndio</h1>
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Cadastro de brigada e validade do treinamento</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="rounded-xl p-6 space-y-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Responsável *">
              <input name="responsavel" required placeholder="Nome do responsável pela brigada" className="form-input" />
            </FormField>
            <FormField label="Quantidade de Membros">
              <input name="qtdMembros" type="number" min="0" placeholder="Ex: 12" className="form-input" />
            </FormField>
          </div>

          <div className="grid grid-cols-3 gap-4">
            <FormField label="Data do Treinamento">
              <input name="dataTreinamento" type="date" className="form-input" />
            </FormField>
            <FormField label="Validade *">
              <input name="dataValidade" type="date" required className="form-input" />
            </FormField>
            <FormField label="Status">
              <select name="status" className="form-select" defaultValue="ATIVO">
        ...
        <div className="flex justify-end gap-3 mt-4">
          <Link href="/sst/avcb/brigada" className="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-secondary)' }}>
            Cancelar
          </Link>
          <button type="submit" disabled={saving} className="flex items-center gap-2 rounded-lg px-5 py-2 text-sm font-bold text-white" style={{ background: 'var(--brand-gradient)', opacity: saving ? .7 : 1 }}>
            <Save size={14} />
            {saving ? 'Salvando...' : 'Salvar Brigada'}
          </button>
        </div>
      </form>
    </div>
