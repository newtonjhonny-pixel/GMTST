...
export default function NovoSimuladoPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      dataPrevista:   fd.get('dataPrevista') || null,
      dataRealizacao: fd.get('dataRealizacao') || null,
      participantes:  fd.get('participantes') || null,
      status:         fd.get('status'),
      observacao:     fd.get('observacao') || null,
    }
    try {
      const res = await fetch('/api/sst/simulados', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      const data = await res.json()
      if (!res.ok) { setError(data.error ?? 'Erro ao salvar'); setSaving(false); return }
      router.push('/sst/avcb/simulados')
      router.refresh()
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/sst/avcb/simulados" className="flex items-center justify-center rounded-lg" style={{ width: 32, height: 32, background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <ArrowLeft size={14} style={{ color: 'var(--text-secondary)' }} />
        </Link>
        <div>
          <h1 className="text-xl font-extrabold" style={{ color: 'var(--text-primary)', letterSpacing: '-.3px' }}>Novo Simulado de Incêndio</h1>
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Simulado de abandono / emergência</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="rounded-xl p-6 space-y-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div className="grid grid-cols-3 gap-4">
            <FormField label="Data Prevista">
              <input name="dataPrevista" type="date" className="form-input" />
            </FormField>
            <FormField label="Data de Realização">
              <input name="dataRealizacao" type="date" className="form-input" />
            </FormField>
            <FormField label="Participantes">
              <input name="participantes" type="number" min="0" placeholder="Ex: 40" className="form-input" />
            </FormField>
          </div>

          <FormField label="Status">
            <select name="status" className="form-select" defaultValue="PENDENTE">
        ...
        <div className="flex justify-end gap-3 mt-4">
          <Link href="/sst/avcb/simulados" className="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-secondary)' }}>
            Cancelar
          </Link>
