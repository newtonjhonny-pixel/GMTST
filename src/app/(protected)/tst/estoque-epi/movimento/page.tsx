...
type EPI = { id: string; nome: string; ca: string | null; quantidadeEstoque?: number; empresaId?: string | null }

function MovimentoEstoqueEpiForm() {
  ...
  const [form, setForm] = useState({
    ...
    epiId: searchParams.get('epiId') ?? '',
    tipo: 'ENTRADA', quantidade: '1', dataMovimento: '', responsavel: '', motivo: '', observacao: '',
  })
  ...
  const epiSelecionado = epis.find(e => e.id === form.epiId)
  ...
  async function salvar(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setErro(data.error ?? 'Erro ao salvar movimento'); setLoading(false); return }
      router.push(form.epiId ? `/tst/estoque-epi/${form.epiId}` : '/tst/estoque-epi')
      router.refresh()
    } catch {
  ...
  const TIPO_INFO: Record<string, { color: string; bg: string; desc: string }> = {
    ENTRADA: { color: '#16a34a', bg: '#f0fdf4', desc: 'Adiciona ao estoque (compras, reposição)' },
    SAIDA:   { color: '#dc2626', bg: '#fef2f2', desc: 'Remove do estoque manualmente (não use para entregas — isso é automático via ficha)' },
    AJUSTE:  { color: '#d97706', bg: '#fffbeb', desc: 'Acerto de inventário — substitui o saldo atual pelo valor informado' },
    PERDA:   { color: '#b91c1c', bg: '#fef2f2', desc: 'Remove do estoque por perda, dano ou extravio' },
    BAIXA:   { color: '#475569', bg: '#f1f5f9', desc: 'Remove do estoque por descarte ou fim de vida útil' },
  }
    ...
    <div style={{ maxWidth: 640 }}>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Entrada, saída, ajuste, perda ou baixa de EPI</p>
        </div>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 20px' }}>
            ...
            {epiSelecionado && (
              <div style={{ gridColumn: 'span 2', fontSize: 12, color: 'var(--text-muted)' }}>
                Estoque atual: <strong style={{ color: 'var(--text-primary)' }}>{epiSelecionado.quantidadeEstoque ?? 0}</strong>
              </div>
            )}
            ...
            {field(<>{label('Motivo')}<input className="form-input" placeholder="Ex: Compra, inventário, descarte..." value={form.motivo} onChange={e => set('motivo', e.target.value)} /></>, 2)}
