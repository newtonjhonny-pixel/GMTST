...
export default function NovaLicencaPage() {
  ...
  const [form, setForm] = useState({
    empresaId: '', unidadeId: '', tipo: 'LO', orgao: '', numero: '',
    emissao: '', vencimento: '', responsavel: '', condicionantes: '', status: 'VIGENTE',
  })
  ...
  async function salvar(e: React.FormEvent) {
    e.preventDefault()
    setLoading(true)
    const res = await fetch('/api/meio-ambiente/licencas', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(form),
    })
    setLoading(false)
    if (res.ok) router.push('/meio-ambiente/licencas')
    else alert('Erro ao salvar licença')
  }
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-extrabold" style={{ color: 'var(--text-primary)', letterSpacing: '-.3px' }}>Nova Licença Ambiental</h1>
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>LP, LI, LO, Outorga, CTF/IBAMA e outras licenças</p>
        </div>
      </div>
      ...
      <form onSubmit={salvar}>
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: 24, boxShadow: 'var(--shadow-sm)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 20px' }}>
            ...
            {field(<>{label('Tipo de Licença', true)}<select className="form-select" value={form.tipo} onChange={e => set('tipo', e.target.value)} required>
              ...
              <option value="CTF_IBAMA">CTF/IBAMA</option>
              <option value="AUTORIZACAO">Autorização</option>
              <option value="OUTRO">Outro</option>
            </select></>)}
            {field(<>{label('Órgão Emissor', true)}<input className="form-input" placeholder="Ex: CETESB, SEMAD, IBAMA..." value={form.orgao} onChange={e => set('orgao', e.target.value)} required /></>)}

            {field(<>{label('Número / Processo')} <input className="form-input" placeholder="Nº da licença" value={form.numero} onChange={e => set('numero', e.target.value)} /></>)}
            ...
            {field(<>{label('Condicionantes / Observações')} <textarea className="form-input" rows={3} placeholder="Liste as condicionantes principais ou observações relevantes..." value={form.condicionantes} onChange={e => set('condicionantes', e.target.value)} /></>, 2)}
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 mt-4">
          ...
          <button type="submit" disabled={loading} className="flex items-center gap-2" style={{ padding: '8px 20px', borderRadius: 8, background: 'var(--brand-gradient)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? .7 : 1, border: 'none' }}>
            <Save size={14} />{loading ? 'Salvando...' : 'Salvar Licença'}
          </button>
        </div>
      </form>
    </div>
